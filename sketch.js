...
function run() {
  ...
  if (level.finish) {
    ...
    const fx = level.finish.x;
    const fy = level.finish.y;
    const fw = level.finish.w;
    const fh = level.finish.h;

    const touching =
      px < fx + fw && px + pw > fx && py < fy + fh && py + ph > fy;
...
function keyPressed() {
  if (currentState === "play" || currentState == "fields") {
    if (key === " " || key === "W" || key === "w" || keyCode === UP_ARROW) {
      player.tryJump();
    }
  }

  if (currentState === "dead" && (key === "r" || key === "R")) {
    levelIndex = 0;
    currentState = "play";
    loadLevel(levelIndex);
    doomBlock = new DoomBlock(0, -800, 0.5, level.w, 500);
  ...
  if (currentState === "fields" && (key === "r" || key === "R")) {
    levelIndex = 0;
    currentState = "play";
    loadLevel(levelIndex);
    doomBlock = new DoomBlock(0, -800, 0.5, level.w, 500);
